...
export default function NewAssetPage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const res = await apiFetch("/api/asset-rt/categories");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/asset-rt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!data.success) {
        setSubmitError(data.error?.message ?? "Gagal menambahkan aset");
        return;
      }
      router.push("/asset-rt");
      router.refresh();
    } catch (err) {
      setSubmitError(
        err instanceof Error ? err.message : "Gagal menambahkan aset",
      );
    } finally {
      setIsSubmitting(false);
    }
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="mt-1 text-[13px] text-white/60">
              Isi detail aset baru untuk inventarisasi
            </p>
          </div>
        </section>
