...
export default function KasRtNewPage() {
  ...
  }, [selectedBlok, iplCategory, monthName]);
  ...
  const submitTransaction = useCallback(
    async (override: boolean) => {
      ...
      try {
        ...
        if (res.status === 409) {
          ...
          toast.error(body.message ?? "Sudah ada pembayaran IPL untuk blok ini.");
          return;
        }
    ...
    [selectedBlok, amount, customTitle, customDescription],
  );
    ...
    <main className="flex min-h-screen flex-col bg-app-surface-alt">
      ...
      >
                    ...
                    {list.map((house) => {
                      ...
                      return (
                        <button
                          key={house.id}
                          type="button"
                          onClick={() => setSelectedBlok(house.blok_rumah)}
                          className={`flex aspect-square items-center justify-center rounded-xl text-sm font-semibold transition-all duration-150 ${
                            isSelected
                              ? "bg-app-primary text-white shadow-md shadow-app-primary/25 ring-2 ring-app-primary ring-offset-2 ring-offset-app-surface-alt"
        ...
        <section className="mb-8">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-app-border bg-app-surface shadow-sm">
            {/* Receipt header */}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-app-border bg-app-surface-alt/90 px-5 py-4 backdrop-blur-md">
        <div className="mx-auto max-w-[430px]">
          <PrimaryButton
            ...
            onPress={() => void handleSubmit(new Event("submit") as unknown as React.FormEvent)}
          >
            Simpan IPL
          </PrimaryButton>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-5 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-3xl bg-app-surface p-6 shadow-xl">
            <div className="mb-4 text-center">
              ...
              <p className="mt-1 text-sm text-app-body-muted">
                Blok {selectedBlok} sudah tercatat membayar IPL bulan {monthName}{" "}
                ini.
              </p>
            </div>
